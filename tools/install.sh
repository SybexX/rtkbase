...
man_help(){
    ...
    echo ''
    echo '        -r | --rtklib'
    echo '                         Get RTKlib 2.4.3b34k from github and compile it.'
    echo '                         https://github.com/rtklibexplorer/RTKLIB/tree/b34k'
    echo ''
...
install_rtklib() {
    ...
    if printf '%s\0' "${sbc_array[@]}" | grep -Fxqz -- "${computer_model}" \
        && [[ -f "${rtkbase_path}"'/tools/bin/rtklib_b34k/'"${arch_package}"'/str2str' ]] \
        && lsb_release -c | grep -qE 'bullseye|bookworm' \
        && "${rtkbase_path}"'/tools/bin/rtklib_b34k/'"${arch_package}"/str2str --version > /dev/null 2>&1
    then
      echo 'Copying new rtklib binary for ' "${computer_model}" ' - ' "${arch_package}"
      cp "${rtkbase_path}"'/tools/bin/rtklib_b34k/'"${arch_package}"/str2str /usr/local/bin/
      cp "${rtkbase_path}"'/tools/bin/rtklib_b34k/'"${arch_package}"/rtkrcv /usr/local/bin/
      cp "${rtkbase_path}"'/tools/bin/rtklib_b34k/'"${arch_package}"/convbin /usr/local/bin/
    else
...
_compil_rtklib() {
    echo '################################'
    echo 'COMPILING RTKLIB 2.4.3 b34k'
    echo '################################'
    #Get Rtklib 2.4.3 b34k release
    sudo -u "${RTKBASE_USER}" wget -qO - https://github.com/rtklibexplorer/RTKLIB/archive/refs/tags/b34k.tar.gz | tar -xvz
    #Install Rtklib app
    #TODO add correct CTARGET in makefile?
    make --directory=RTKLIB-b34k/app/consapp/str2str/gcc
    make --directory=RTKLIB-b34k/app/consapp/str2str/gcc install
    make --directory=RTKLIB-b34k/app/consapp/rtkrcv/gcc
    make --directory=RTKLIB-b34k/app/consapp/rtkrcv/gcc install
    make --directory=RTKLIB-b34k/app/consapp/convbin/gcc
    make --directory=RTKLIB-b34k/app/consapp/convbin/gcc install
    #deleting RTKLIB
    rm -rf RTKLIB-b34k/
}
